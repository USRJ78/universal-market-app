...
def run_backtest():
    print("==================================================")
    print("📈 SUPER-OPTIMIZED AI CORRELATION OPTIONS STRATEGY 📈")
    print("==================================================")
    ...
    for w in [10, 50, 120]:
        ...
        df[f'Corr_{w}'] = df['BTC_Ret'].rolling(window=w).corr(df['ETH_Ret'])
        
    # Volatility
    df['BTC_Vol'] = df['BTC_Ret'].rolling(window=50).std() * math.sqrt(365 * 24)
    df['ETH_Vol'] = df['ETH_Ret'].rolling(window=50).std() * math.sqrt(365 * 24)
    ...
    # Target labeling (48 hours lookahead for cleaner signal)
    print("Building training labels (48h lookahead)...")
    lookahead = 48
    labels = []
    for i in range(len(df)):
        if i + lookahead < len(df):
            ...
            if ratio_change > 0.025:
                labels.append(1) # BTC outperforming
            elif ratio_change < -0.025:
    ...
    # 4. Train model
    print("Training PyTorch Correlation NN (11 Features, BatchNorm)...")
    ...
    # 5. Options Simulation Backtest (Long Asymmetric Options)
    print("Running Long Options Spreads Backtest on test set...")
    model.eval()
    with torch.no_grad():
    ...
    rf_rate = 0.05
    allocation_pct = 0.05 # Allocate 5% to each trade
    confidence_threshold = 0.60 # Strong confidence required
    ...
    for i in range(len(test_df)):
        ...
        for trade in active_trades:
            if i >= trade['expiry_index']:
                ...
                if trade['type'] == 1:
                    ...
                    btc_return = (btc_payoff / (trade['btc_premium'] + 1e-9))
                    eth_return = (eth_payoff / (trade['eth_premium'] + 1e-9))
                    
                    payoff = (trade['allocation'] / 2.0) * btc_return + (trade['allocation'] / 2.0) * eth_return
                    
                # Trade Type 2: Buy BTC Put Spread & Buy ETH Call Spread
                elif trade['type'] == 2:
        ...
        # 2. Place new trades based on signal
        if signal in [1, 2] and prob >= confidence_threshold and len(active_trades) < 10 and capital > 100:
    ...
    plt.plot(test_df.index, test_df['Strategy_Equity'], label='Optimized AI Options Strategy', color='blue', linewidth=2)
    plt.plot(test_df.index, test_df['BTC_Hold_Return'], label='BTC Buy & Hold', color='orange', linestyle='--', alpha=0.7)
    plt.title('Super-Optimized AI Correlation Options Strategy vs BTC Buy & Hold')
    plt.xlabel('Date')
    plt.ylabel('Equity ($)')
    plt.legend()
    plt.grid(True)
    
    chart_path = r'C:\Users\USER\.gemini\antigravity\brain\bcb2ab91-bc57-4f82-bd4e-b0bf91d0de91\correlation_nn_optimized_chart.png'
    plt.savefig(chart_path)
    print(f"Performance chart saved to {chart_path}")
    
    # Save Report
    report = f"""# Super-Optimized AI Correlation Options Strategy (11 Features, 48h Labeling)

We evaluated the performance of the **Super-Optimized AI Correlation Options Strategy** which utilizes a multi-timeframe feature space, 48-hour future labeling to reduce noise, and confidence-filtered asymmetric option buying.
...
The performance chart is saved at [correlation_nn_optimized_chart.png](file:///{chart_path.replace('\\', '/')}).
"""
    report_path = r'C:\Users\USER\.gemini\antigravity\brain\bcb2ab91-bc57-4f82-bd4e-b0bf91d0de91\correlation_nn_optimized_report.md'
